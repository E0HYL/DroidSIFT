...
public class MyReachingDefinitionAnalysis extends ForwardFlowAnalysis {
	...
	public MyReachingDefinitionAnalysis(UnitGraph g) {
		...
		// together
		{
			Map<Local, ArrayList> localToDefList = new HashMap<Local, ArrayList>(
					g.getBody().getLocalCount() * 2 + 1, 0.7f);
			...
			// Fill the sets up
			{
				...
				while (it.hasNext()) {
					...
					if (!defBoxes.isEmpty()) {
						...
						//System.out.println("In MyReachingDefinitionAnalysis: " + s + "|" + ((ValueBox) defBoxes.get(0)).getValue());
						if (((ValueBox) defBoxes.get(0)).getValue() instanceof Local) {
							//System.out.println("In MyReachingDefinitionAnalysis/Local: " + s + "|" + ((ValueBox) defBoxes.get(0)).getValue());
							
							Local defLocal = (Local) ((ValueBox) defBoxes
									.get(0)).getValue();
							// if(defLocal.getType() instanceof RefLikeType){
							// System.out.println(defLocal + " of type " +
							// defLocal.getType());
							// }
							List<Unit> l = localToDefList.get(defLocal);

							if (l == null)
								throw new RuntimeException("local " + defLocal
										+ " is used but not declared!");
							else
								l.add(s);
						}
						/*
						else if(((ValueBox) defBoxes.get(0)).getValue() instanceof InstanceFieldRef){
							
							if(((InstanceFieldRef) ((ValueBox) defBoxes.get(0)).getValue()).getBase() instanceof Local){
							
								System.out.println("In MyReachingDefinitionAnalysis/InstanceFieldRef: " + s + "|" + ((InstanceFieldRef)((ValueBox) defBoxes.get(0)).getValue()).getBase());
								Local defLocal = (Local)((InstanceFieldRef) ((ValueBox) defBoxes
										.get(0)).getValue()).getBase();
								// if(defLocal.getType() instanceof RefLikeType){
								// System.out.println(defLocal + " of type " +
								// defLocal.getType());
								// }
			...
			// Generate the list & localToIntPair
			{
				...
				localToIntPair = new HashMap<Local, IntPair>(g.getBody()
		...
		{
			Map<Local, FlowSet> localToKillSet = new HashMap<Local, FlowSet>(g
					.getBody().getLocalCount() * 2 + 1, 0.7f);
			localToPreserveSet = new HashMap<Local, BoundedFlowSet>(g.getBody()
					.getLocalCount() * 2 + 1, 0.7f);
			...
			for (Object element : defs) {
				...
				if (((ValueBox) defBoxes.get(0)).getValue() instanceof Local) {
					Local defLocal = (Local) ((ValueBox) defBoxes.get(0))
							.getValue();
					BoundedFlowSet killSet = (BoundedFlowSet) localToKillSet
							.get(defLocal);
	...
	protected void flowThrough(Object inValue, Object d, Object outValue) {
		...
		if (!defBoxes.isEmpty()) {
			...
			/*
				...
				if(((InstanceFieldRef) value).getBase() instanceof Local){
					Local defLocal = (Local)( ((InstanceFieldRef) value).getBase());
